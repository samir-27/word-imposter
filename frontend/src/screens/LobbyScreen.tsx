...
export const LobbyScreen: React.FC<LobbyScreenProps> = ({ room, onLeaveRoom, onStartGame }) => {
  ...
  // Build join URL with pre-filled room code
  const joinUrl = `${window.location.protocol}//${window.location.host}?code=${room.roomCode}`;
  ...
  return (
    <div className="space-y-5">
      ...
      {/* QR Code Popup / View */}
      {showQr && (
        <div className="p-4 rounded-xl bg-white flex flex-col items-center justify-center space-y-2 text-slate-900 animate-fade-in shadow-xl">
          <QRCodeSVG value={joinUrl} size={160} level="M" />
          <p className="text-[11px] font-bold text-slate-600 text-center">
            Scan with phone camera on same Wi-Fi to join!
          </p>
        </div>
      )}
